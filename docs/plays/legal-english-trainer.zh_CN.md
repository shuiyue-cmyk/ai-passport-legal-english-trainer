...
息屏后继续闲置 10 分钟自动存盘进深度睡眠（约 0.3mA≈关机），按任意键唤醒（=重启，1-2 秒，进度无感恢复）。
...
| 操作 | 行为 |
| --- | --- |
| OK 单击 | 未翻面：显示答案；已翻面：下一词 |
| OK 双击 | 播放当前词发音 |
| OK 长按 | 返回主菜单 |
| ↑ / ↓ 单击 | 上/下一个词（翻面与否一样，只看不记分） |
| ↑ / ↓ 双击 | 跳 10 条 |
| ↑ / ↓ 长按 | 跳到相邻章节的第一条 |

有例句的词条在卡片下方显示教材原句（英文+中文翻译，1253/1639 条覆盖）；
英文例句常态显示，中文例句仅看到答案后显示（卡片翻面、选择题判分后）。
长文本首停 3 秒后单行来回弹滚动，短文本静止。
新词条出现时自动播放一遍发音（翻面/判分不重播）。

### 选择题（混合练习、错词本）

屏上给出英文，下方 4 个中文备选（3 个全库随机干扰项），末行是同行的**不认识**与**播放音频**；
↑↓ 选择、OK 确认。新题出现时自动播放一遍发音。
...
- **学习单词**：按主章节锁死分批（20 个一批，末批 9 个），批内打乱顺序；
  从首个有生词的批开始，做完自动进下一批，批号与批内进度都存 NVS（中途退出重进可续）。
...
学习答对、错词答对的词，按 1/4 概率攒 30 累计学习分钟后到期（其余直接毕业），
每次取到期最早的 20 个；同一词两次复习至少间隔一组（上组刚复习过的不进下组）。

| 阶段 | 行为 |
| --- | --- |
| 认不认识 | 只给英文（+英文例句）：认识亮中文，OK 进四选一；不认识不亮答案，直接往后放一轮四选一 |
| 四选一 | 与选择题同规则；答对按 1/4 概率顺延 30 分钟（未抽中即毕业），答错扔回错词本 |
...
- **错词本** = 选择题/复习里答错过、且尚未练到「熟练」的词（卡片只看不记分，不进错词本），
  在错词本里以选择题重练。
...
| 词库数据（内嵌 app） | 1639 条，`vocab_data.c` 约 510 KB |
| 中文字体（内嵌 app） | 黑体 16px 子集，1737 个字符 |
| Opus 音频分区 `vocabfs` | 1639 条 / 101,252 个 Opus 包 / **约 1.9 MB** |
